...
import { Cpu, Activity, AlertTriangle, ArrowRight, Sparkles } from "lucide-react";
...
const trend = Array.from({ length: 24 }, (_, i) => ({
  v: 50 + Math.sin(i / 2) * 12 + Math.random() * 6,
}));
        ...
        <div className="rounded-xl p-6 border border-border glass-strong relative overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-6 flex-wrap">
            ...
            <div className="grid grid-cols-2 gap-3 min-w-[300px]">
              ...
              <Kpi icon={BrandBolt} label="Consumo" v="312 kW" tone="primary" />
          ...
          <div className="lg:col-span-2 rounded-xl border border-border bg-card p-5">
            ...
            <div className="space-y-2">
              {[
                ...
                >
                  <span
                    className={`h-2 w-2 rounded-full ${p.st === "online" ? "bg-success" : "bg-warning"} energized`}
                  />
...
function Kpi({ icon: Icon, label, v, tone }: any) {
  const colors: any = { success: "text-success", primary: "text-primary", warning: "text-warning" };
  return (
...
function ChartCard({ title, color }: { title: string; color: string }) {
  const data = Array.from({ length: 30 }, (_, i) => ({
    v: 30 + Math.sin(i / 2) * 10 + Math.random() * 8,
  }));
  return (
