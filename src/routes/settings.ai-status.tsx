...
export const Route = createFileRoute("/settings/ai-status")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Diagnóstico da integração com o motor de IA (DeepSeek): chave, ping e histórico.",
      },
    ],
  }),
...
function AiStatusPage() {
  ...
  const refresh = async () => {
    ...
    setEvents(await fetchAiStatusEvents());
    setBusy(false);
  };
  ...
  useEffect(() => {
    refresh();
    const onEv = () => {
      void fetchAiStatusEvents().then(setEvents);
    };
      ...
      <div className="max-w-[900px] mx-auto p-6 space-y-6">
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Provedor: <span className="font-mono text-foreground">DeepSeek</span> · modelo{" "}
              <span className="font-mono text-foreground">deepseek-chat</span>
            </p>
          </div>
          <button
            onClick={refresh}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Card
            ...
            tone={health?.keyConfigured ? "ok" : "err"}
            value={
              health?.keyConfigured
                ? health?.keyFormatValid
                  ? "OK"
                  : "Formato inválido"
                : "Ausente"
            }
          ...
          <Card
            icon={CheckCircle2}
            label="Último ping ao DeepSeek"
            tone={health?.pingOk ? "ok" : health == null ? "warn" : "err"}
            value={
              health?.pingOk ? `200 OK` : health?.pingStatus ? `HTTP ${health.pingStatus}` : "—"
            }
              ...
              <div className="flex-1">
                ...
                <ol className="list-decimal list-inside mt-2 space-y-1.5 text-sm text-foreground/90">
                  <li>
                    ...
                    >
                      platform.deepseek.com/api_keys <ExternalLink className="h-3 w-3" />
                    </a>{" "}
                    e confirme que sua conta tem créditos.
                  </li>
                  <li>
                    No Supabase, abra{" "}
                    <a
                      ...
                      className="underline inline-flex items-center gap-1 text-primary"
                    >
                      Settings → Edge Functions → Secrets <ExternalLink className="h-3 w-3" />
                    </a>{" "}
                    e atualize{" "}
                  ...
                  <li>
                    Volte aqui e clique em <strong>Revalidar agora</strong>.
                  </li>
                </ol>
                <p className="mt-3 text-[12px] text-muted-foreground">
                  Por segurança, sua chave nunca aparece nesta página nem em logs do app.
                </p>
              </div>
            </div>
          </div>
        ...
        <section>
              ...
              <tbody>
                ...
                {events.map((e, i) => (
                  <tr key={i} className="border-t border-border">
                    ...
                    <td
                      className={`px-3 py-1.5 font-medium ${e.ok ? "text-success" : "text-destructive"}`}
                    >
                      {e.ok ? "✓ OK" : "✗ Falha"}
                    </td>
          ...
          {lastEvent && (
            <p className="mt-2 text-[11px] text-muted-foreground">
              Última atividade: {new Date(lastEvent.ts).toLocaleString()}
            </p>
          )}
        </section>
      </div>
