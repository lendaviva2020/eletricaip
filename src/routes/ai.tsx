...
function AiPage() {
  ...
  const [history, setHistory] = useState<ChatMsg[]>([
    { role: "ai", text: "Descreva uma planta, um equipamento ou só um briefing curto. Eu projeto o sistema elétrico completo: trafo, CCM, motores, proteções, cabos, instrumentação e topologia. Tudo aparece simultaneamente em Unifilar, Ladder, FBD, SCADA e Twin." },
  ]);
      ...
      <div className="flex-1 flex flex-col">
        <div className="px-6 py-4 border-b border-border">
          <h1 className="text-lg font-semibold flex items-center gap-2">
            ...
            <span className="text-[9px] font-mono uppercase tracking-wider text-primary/80 ml-2 px-1.5 py-0.5 rounded bg-primary/10">CORE ENGINE</span>
          </h1>
        ...
        <div className="flex-1 overflow-auto scrollbar-thin p-6 space-y-4 max-w-3xl mx-auto w-full">
          ...
          {history.length <= 2 && (
            <div className="grid sm:grid-cols-2 gap-2 pt-4">
              {SUGGESTIONS.map((s) => (
                <button key={s} onClick={() => send(s)} className="text-left text-[12px] p-3 rounded-md border border-border hover:border-primary/40 hover:bg-accent/40 transition-colors">
...
function ResultCard({ result, onApply }: { result: ArchitectResult; onApply: (r: ArchitectResult, mode: "ladder" | "unifilar" | "both") => void }) {
  ...
  return (
    <div className="mt-4 space-y-3">
      <div className="grid sm:grid-cols-3 gap-2">
        <Stat label="Trafo" value={`${result.transformer.kVA} kVA`} sub={`${result.transformer.primary_kV}/${result.transformer.secondary_V}V`} />
        <Stat label="Motores" value={`${result.motors.length}`} sub={`Σ ${demand.totalLoad_kW} kW`} />
        <Stat label="Corrente nominal" value={`${Math.round(totalIn)} A`} sub={`Demanda ${demand.apparent_kVA} kVA`} />
        <Stat label="CCM" value={`${result.ccm.columns} col.`} sub={`${result.ccm.cells} células`} />
        <Stat label="Nós" value={`${result.nodes.length}`} sub={`${result.edges.length} ligações`} />
        <Stat label="Trafo sugerido" value={`${demand.transformer_kVA} kVA`} sub="cálc. em memória" />
      </div>
        ...
        <div className="mt-2 max-h-48 overflow-auto scrollbar-thin">
          <table className="w-full text-[11px] font-mono">
            <thead className="text-muted-foreground"><tr>
              <th className="text-left p-1">ID</th><th className="text-right p-1">P (kW)</th>
              <th className="text-right p-1">In (A)</th><th className="text-right p-1">Ist (A)</th>
              <th className="text-right p-1">Cabo</th><th className="text-right p-1">Disj.</th>
