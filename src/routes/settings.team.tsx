...
interface DemoMember {
  id: string; name: string; email: string; role: Role; joinedAt: string; active?: boolean;
}
interface DemoInvite {
  id: string; email: string; role: Role; createdAt: string;
}
interface DemoWorkspace {
  id: string; name: string; plan: string; role: Role; isActive: boolean;
}
...
const DEMO_ACTIVITIES = [
  { id: "a1", icon: <User className="h-2.5 w-2.5" />, label: "Ana Silva entrou", desc: "Engenharia · Acesso via SSO", time: "2 min atrás", status: "ok" as const },
  { id: "a2", icon: <Bot className="h-2.5 w-2.5" />, label: "IA executou automação", desc: "Diagnóstico PLC-07 concluído", time: "5 min atrás", status: "ok" as const },
  { id: "a3", icon: <Activity className="h-2.5 w-2.5" />, label: "Projeto alterado", desc: "Subestação Sul — revisão de normas", time: "12 min atrás", status: "warn" as const },
  { id: "a4", icon: <Wifi className="h-2.5 w-2.5" />, label: "PLC conectado", desc: "CLP-03 · Modbus TCP ativo", time: "18 min atrás", status: "ok" as const },
  { id: "a5", icon: <AlertCircle className="h-2.5 w-2.5" />, label: "Alerta criado", desc: "Temperatura acima do limite no motor M-02", time: "25 min atrás", status: "err" as const },
];
...
function TeamPage() {
  ...
  const [demoMembers, setDemoMembers] = useState<DemoMember[]>(() => [
    { id: genId(), name: "Ana Silva", email: "ana@eletricai.com", role: "admin", joinedAt: new Date(Date.now() - 120 * 86400000).toISOString(), active: true },
    { id: genId(), name: "Carlos Oliveira", email: "carlos@eletricai.com", role: "engineer", joinedAt: new Date(Date.now() - 90 * 86400000).toISOString(), active: true },
    { id: genId(), name: "Mariana Costa", email: "mariana@eletricai.com", role: "engineer", joinedAt: new Date(Date.now() - 60 * 86400000).toISOString(), active: false },
    { id: genId(), name: "João Santos", email: "joao@eletricai.com", role: "operator", joinedAt: new Date(Date.now() - 30 * 86400000).toISOString(), active: true },
    { id: genId(), name: "Paula Ferreira", email: "paula@eletricai.com", role: "viewer", joinedAt: new Date(Date.now() - 15 * 86400000).toISOString(), active: true },
    { id: genId(), name: "Ricardo Lima", email: "ricardo@eletricai.com", role: "engineer", joinedAt: new Date(Date.now() - 7 * 86400000).toISOString(), active: false },
  ]);
  ...
  const { data: memberList = [], isLoading: membersLoading } = useQuery({
    ...
    queryFn: async () => {
      try { return await membersFn({}); }
      catch { setBackendError(true); return []; }
    },
  ...
  const handleInvite = useCallback(async () => {
    ...
    try {
      if (isDemo) {
        ...
        setDemoInvites((prev) => [{ id: genId(), email: email.trim(), role, createdAt: new Date().toISOString() }, ...prev]);
  ...
  const handleUpdateRole = useCallback(async (userId: string, newRole: Role) => {
    setBusy(`role:${userId}`);
    try {
      if (isDemo) {
        await new Promise((r) => setTimeout(r, 300));
        setDemoMembers((prev) => prev.map((m) => (m.id === userId ? { ...m, role: newRole } : m)));
  ...
  const handleWorkspaceSwitch = useCallback(async (id: string) => {
    if (isDemo) {
      setDemoMembers([]); setDemoInvites([]);
      toast.success("Workspace alterado (simulação).");
      return;
    }
    try { await switchTo(id); toast.success("Workspace alterado."); }
    catch (e) { toast.error((e as Error).message); }
  }, [isDemo, switchTo]);
  ...
  const filteredMembers = displayMembers.filter((m: any) =>
    !memberSearch.trim() || (m.name || m.full_name || "").toLowerCase().includes(memberSearch.toLowerCase()) ||
    (m.email || "").toLowerCase().includes(memberSearch.toLowerCase()),
  );

  const orgTree = [
    { id: "empresa", label: "EletricAI Industrial", icon: Building2, children: [
  ...
  const permRows = [
    ...
    { label: "Normas técnicas", key: "normas", view: true, edit: true, execute: false, admin: false },
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground/60">Gestão de equipe · {active?.name ?? "EletricAI Industrial"}</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className={`h-2 w-2 rounded-full ${onlineCount > 0 ? "bg-emerald-500 animate-pulse" : "bg-muted-foreground/30"}`} />
          ...
          <Button size="sm" variant="outline" className="h-8 gap-1.5 text-xs" onClick={() => setPermOpen(true)}>
        ...
        <div className="max-w-6xl mx-auto p-6">
          ...
          <div className="grid grid-cols-4 gap-3 mb-6">
            <div className="rounded-xl border border-border/40 p-3.5 bg-gradient-to-b from-background/80 to-muted/10">
              <span className="text-[10px] text-muted-foreground/60 uppercase tracking-wider">Membros</span>
              <p className="text-xl font-semibold mt-1">{displayMembers.length} <span className="text-xs text-muted-foreground/60 font-normal">total</span></p>
            </div>
            <div className="rounded-xl border border-border/40 p-3.5 bg-gradient-to-b from-background/80 to-muted/10">
              <span className="text-[10px] text-muted-foreground/60 uppercase tracking-wider">Atividade</span>
              <p className="text-xl font-semibold mt-1">{onlineCount} <span className="text-xs text-muted-foreground/60 font-normal">online</span></p>
            </div>
            <div className="rounded-xl border border-border/40 p-3.5 bg-gradient-to-b from-background/80 to-muted/10">
              <span className="text-[10px] text-muted-foreground/60 uppercase tracking-wider">Convites</span>
              <p className="text-xl font-semibold mt-1">{displayInvites.length} <span className="text-xs text-muted-foreground/60 font-normal">pendentes</span></p>
            </div>
            <div className="rounded-xl border border-border/40 p-3.5 bg-gradient-to-b from-background/80 to-muted/10">
              <span className="text-[10px] text-muted-foreground/60 uppercase tracking-wider">Workspaces</span>
              <p className="text-xl font-semibold mt-1">{workspaces.length} <span className="text-xs text-muted-foreground/60 font-normal">ativos</span></p>
            </div>
          </div>
            ...
            <div>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-xs transition-all ${
                          isSelected ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground/70 hover:bg-muted/30"
                        }`}
                      >
                        <ChildIcon className={`h-3.5 w-3.5 ${isSelected ? "text-primary" : ""}`} />
                        <span className="flex-1 text-left">{child.label}</span>
                        <span className={`text-[10px] ${isSelected ? "text-primary/70" : "text-muted-foreground/40"}`}>
                          {child.count}
                        </span>
                      </button>
                    );
              ...
              <div className="space-y-2">
                {workspaces.map((w: any) => {
                  ...
                  return (
                    <div key={w.id} className={`rounded-lg border p-3 transition-all ${isActive ? "border-primary/30 bg-primary/[0.03]" : "border-border/30 hover:border-border/60"}`}>
                      <div className="flex items-center gap-2.5">
                        <div className={`flex h-7 w-7 items-center justify-center rounded-md ${isActive ? "bg-primary/10" : "bg-muted/30"}`}>
                          <Building2 className={`h-3.5 w-3.5 ${isActive ? "text-primary" : "text-muted-foreground/50"}`} />
                        </div>
                        ...
                        {isActive ? (
                          <Badge variant="default" className="text-[8px] h-4 gap-0.5"><Check className="h-2 w-2" /></Badge>
                        ) : (
                          <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => handleWorkspaceSwitch(w.id)}>
                ...
                <div className="flex items-center gap-2">
                  <div className="relative">
                    ...
                    <Input value={memberSearch} onChange={(e) => setMemberSearch(e.target.value)}
                      placeholder="Buscar membro..." className="h-7 w-36 pl-7 text-[10px] rounded-md" />
                  </div>
                </div>
              </div>
                      ...
                      >
                        <div className="flex items-center gap-3.5">
                          <div className="relative shrink-0">
                            ...
                            <span className={`absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-background ${isOnline ? "bg-emerald-500" : "bg-muted-foreground/30"}`} />
                          </div>
                          <div className="flex-1 min-w-0 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                            <div className="sm:col-span-2">
                              <div className="flex items-center gap-2">
                                <p className="font-medium truncate">{m.full_name || m.name || m.user_id?.slice(0, 8)}</p>
                                {isCurrentUser && <Badge variant="secondary" className="text-[8px] h-3.5">você</Badge>}
                              </div>
                              <p className="text-[10px] text-muted-foreground/50 truncate mt-0.5">{m.email}</p>
                            </div>
                            <div>
                              <span className="text-[10px] text-muted-foreground/40 block">Setor</span>
                              <span className="text-[11px] font-medium capitalize">{selectedOrg}</span>
                            </div>
                            <div>
                              <span className="text-[10px] text-muted-foreground/40 block">Acesso</span>
                              <span className="text-[11px] font-medium capitalize">{ROLE_CONFIG[memberRole]?.label}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            {!isCurrentUser && (
                              ...
                              >
                                {busy === `kick:${m.user_id || m.id}` ? <Loader2 className="h-3 w-3 animate-spin" /> : <Trash2 className="h-3 w-3" />}
                              </Button>
                            )}
                          </div>
                        </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="space-y-2">
                    {displayInvites.map((i: any) => {
                      ...
                      return (
                        <div key={i.id} className="rounded-lg border border-dashed border-border/40 p-3 flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground/50">
                              ...
                              {i.createdAt && <><span>·</span><Clock className="h-2.5 w-2.5" />{new Date(i.createdAt).toLocaleDateString("pt-BR")}</>}
                            </div>
                          </div>
                          <div className="flex items-center gap-1">
                            <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => {
                              navigator.clipboard.writeText(`${window.location.origin}/invite/${i.token || i.id}`);
                              toast.success("Link copiado");
                            }}>
                              <Copy className="h-3 w-3" />
                            </Button>
                            <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-destructive" onClick={() => handleRevoke(i.id)} disabled={busy === `revoke:${i.id}`}>
                              {busy === `revoke:${i.id}` ? <Loader2 className="h-3 w-3 animate-spin" /> : <X className="h-3 w-3" />}
                            </Button>
                          </div>
                        </div>
                      );
            ...
            <div>
              ...
              <div className="mt-4 rounded-xl border border-border/40 p-3.5 bg-gradient-to-br from-primary/[0.02] to-transparent">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] uppercase tracking-[0.1em] text-muted-foreground/50 font-medium">Uso de IA por membro</span>
                </div>
                {demoMembers.slice(0, 4).map((m) => (
                  <div key={m.id} className="flex items-center gap-2 py-1.5">
                    ...
                    <span className="text-[10px] flex-1 truncate text-muted-foreground/70">{m.name.split(" ")[0]}</span>
                    <span className="text-[10px] font-mono text-primary/80">{Math.floor(Math.random() * 40 + 10)}%</span>
                    <div className="w-12 h-1.5 rounded-full bg-muted/40 overflow-hidden">
                      <div className="h-full rounded-full bg-primary/60" style={{ width: `${Math.floor(Math.random() * 60 + 20)}%` }} />
                    </div>
                  </div>
                ))}
              </div>
      ...
      <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>O convidado receberá um link por e-mail para acessar o workspace.</DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4">
            <div className="space-y-3">
              <div>
                <Label className="text-[10px] text-muted-foreground/70">Nome completo</Label>
                <Input value={inviteName} onChange={(e) => setInviteName(e.target.value)} placeholder="Maria Souza" className="h-9 text-xs mt-1" />
              </div>
              <div>
                <Label className="text-[10px] text-muted-foreground/70">E-mail</Label>
                <Input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="maria@empresa.com" className="h-9 text-xs mt-1" />
              </div>
              <div>
                ...
                <Select value={inviteSector} onValueChange={setInviteSector}>
                  ...
                  <SelectContent>
                    <SelectItem value="engenharia" className="text-xs">Engenharia</SelectItem>
                    <SelectItem value="automacao" className="text-xs">Automação</SelectItem>
                    <SelectItem value="manutencao" className="text-xs">Manutenção</SelectItem>
                    <SelectItem value="supervisao" className="text-xs">Supervisão</SelectItem>
            ...
            <div className="rounded-lg border border-border/40 p-3 bg-muted/10">
              <span className="text-[10px] uppercase tracking-[0.1em] text-muted-foreground/50 font-medium block mb-2">Preview da permissão</span>
              <div className="space-y-1.5 text-[11px]">
                <div className="flex items-center gap-2 text-emerald-500"><CheckCircle2 className="h-3 w-3" /> Visualizar projetos</div>
                <div className={`flex items-center gap-2 ${role !== "viewer" ? "text-emerald-500" : "text-muted-foreground/30"}`}><CheckCircle2 className="h-3 w-3" /> Editar normas</div>
                <div className={`flex items-center gap-2 ${role === "admin" || role === "engineer" || role === "operator" ? "text-emerald-500" : "text-muted-foreground/30"}`}><CheckCircle2 className="h-3 w-3" /> Executar SCADA</div>
                <div className={`flex items-center gap-2 ${role === "admin" ? "text-emerald-500" : "text-muted-foreground/30"}`}><CheckCircle2 className="h-3 w-3" /> Administrar sistema</div>
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" size="sm" onClick={() => setInviteOpen(false)} className="text-xs">Cancelar</Button>
            <Button size="sm" onClick={handleInvite} disabled={busy === "invite"} className="gap-1.5 text-xs">
              {busy === "invite" ? <Loader2 className="h-3 w-3 animate-spin" /> : <UserPlus className="h-3 w-3" />}
              Enviar convite
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={permOpen} onOpenChange={setPermOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>Configure as permissões granulares para cada papel da equipe.</DialogDescription>
          </DialogHeader>
          <PermissionMatrix rows={permRows} />
          <DialogFooter>
            <Button variant="outline" size="sm" onClick={() => setPermOpen(false)} className="text-xs">Fechar</Button>
            <Button size="sm" className="text-xs gap-1.5"><Check className="h-3 w-3" /> Salvar permissões</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
