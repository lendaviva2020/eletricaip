...
export const getProjectExportData = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: project, error: pErr }, { data: diagrams }, { data: bom }] =
      await Promise.all([
        supabase
          .from("projects")
        ...
        supabase
          ...
          .select(
            "part_number, description, manufacturer, quantity, unit, unit_price_brl",
          )
          .eq("project_id", data.projectId),
      ]);
    ...
    const totalBRL = (bom ?? []).reduce(
      (s: number, it: any) =>
        s + Number(it.quantity) * Number(it.unit_price_brl ?? 0),
      0,
    );
    return {
      project,
