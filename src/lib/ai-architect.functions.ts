...
async function fetchNormativeContext(
  ...
  const stop = new Set([
    "para", "com", "uma", "como", "que", "the", "and", "for", "with", "this",
    "esse", "esta", "este", "essa", "isso", "pelo", "pela", "dos", "das",
    "por", "uma", "ser", "são", "tem", "tenho", "preciso", "quero",
  ]);
  const tokens = prompt
    .toLowerCase()
    .normalize("NFD")
  ...
  const chunks = ((data ?? []) as Array<{ category: string | null; chunk_text: string | null }>).map((r) => ({
...
export const generateArchitecture = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<ArchitectOk | ArchitectError> => {
    ...
    if (resp.status === 429)
      return { ok: false, error: { code: "RATE_LIMIT_429", message: "Muitas requisições à IA. Aguarde alguns segundos." } };
    if (resp.status === 402)
      return { ok: false, error: { code: "NO_CREDITS_402", message: "Créditos da conta DeepSeek esgotados." } };
    if (resp.status >= 500) {
      const t = await resp.text();
      console.error("DeepSeek 5xx:", resp.status, t);
      return { ok: false, error: { code: "UPSTREAM_5XX", message: "Serviço DeepSeek temporariamente indisponível." } };
    }
    if (!resp.ok) {
      const t = await resp.text();
      console.error("DeepSeek erro:", resp.status, t);
      return { ok: false, error: { code: "BAD_RESPONSE", message: `Resposta inesperada (${resp.status}).` } };
    }
    ...
    try {
      ...
      return { ok: false, error: { code: "BAD_RESPONSE", message: "JSON inválido devolvido pela IA." } };
    }
