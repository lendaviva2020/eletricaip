...
const series = (n: number, base: number, amp: number) =>
  Array.from({ length: 60 }, (_, i) => ({ t: i, v: base + Math.sin(i / 4 + n) * amp + Math.random() * (amp / 6) }));

export function SimCanvas() {
  return (
    <div className="relative h-full w-full overflow-auto p-6 pt-16 pb-20">
      <FloatingLegend title="Simulação · Tempo Real" items={["Δt 16ms", "Modelo físico", "Solver: RK4", "Live"]}/>
        ...
        {[
          ...
          <div key={s.label} className="rounded-md border border-border glass p-3">
            <div className="flex items-center justify-between text-[11px] mb-1">
              ...
              <span className="font-mono" style={{ color: s.color }}>● live</span>
            </div>
            <div className="h-36">
              <ResponsiveContainer>
                <AreaChart data={s.data}>
                  ...
                  <Area type="monotone" dataKey="v" stroke={s.color} strokeWidth={2} fill={`url(#${s.label})`} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      <BottomStrip items={[["Solver", "RK4"], ["Δt", "16 ms"], ["Steps", "12.4k"], ["Drift", "0.02%"]]} />
    </div>
  );
}
